...
describe('RideHistoryPage', () => {
  beforeEach(async () => {
    jest.useFakeTimers();
    jest.clearAllMocks();
    await setActiveMockAccountId('ride-history-test');
  ...
  it('clears unknown favorite route ids from local storage on load', async () => {
    ...
    await waitFor(async () => {
      expect(await getFavoriteRouteIdsLocal()).toEqual(['1']);
    });
  }, 10000);
  ...
  it('does not allow adding more than 3 unique favorite routes', async () => {
    ...
    expect(await getFavoriteRouteIdsLocal()).toEqual(['1', '2', '3']);
  }, 10000);
  ...
  it('keeps multiple distinct favorites when routes are selected one after another', async () => {
    ...
    await waitFor(async () => {
      expect(await getFavoriteRouteIdsLocal()).toEqual(['1', '2']);
    });
  }, 10000);
  ...
  it('treats multiple ride history entries of the same routeId as one unique favorite', async () => {
    ...
    await waitFor(async () => {
      expect(await getFavoriteRouteIdsLocal()).toEqual(['route-a', 'route-b']);
    });
